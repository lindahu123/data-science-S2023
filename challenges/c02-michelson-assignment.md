...
From examining the table, it looks like the measurements from the more
distinct images are closer to the accepted value for the speed of light.
The table I created differs slightly from Michelson’s table, however the
values are quite similar. One possible explanatation for this would be
that perhaps Michelson excluded a few data points if they were outliers.
...
ModernSpeedOfLight = 299792.458
Error = abs(LIGHTSPEED_VACUUM - LIGHTSPEED_MICHELSON)
paste0("LIGHTSPEED_PM = ", LIGHTSPEED_PM)
```
...
Michelson’s estimation of his error was 51, meanwhile the true value of
the error was over 151. This is over 3 times more than his original
uncertainty range.
...
The graph above shows the simulated data plotted against the real
measured data. At first glance, there seem to be a lot of similarities
between the two. For example, for both there seems to be a similar
amount of standard deviation. There are also slopes in the trendlines
that vaguely match up. The biggest difference between the two graphs, is
that the simulated data stays within Michelson’s estimated error range
more.
...
df_q2 %>% 
  ggplot() +
  geom_point(mapping = aes(x = Distinctness, y = VelocityVacuum))
```

![](c02-michelson-assignment_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
**Observations**:

In the top graph above, Temperature is plotted against VelocityVacuum.
It difficult to determine if there is a statistically significant trend
in the data. It may be possible, since the lowest VelocityVacuum values
occur when the temperature is at it’s lowest. However, there are also
wide deviations in the data for each temperature range, so it’s unclear
whether there this relationship is true or not.

In the second graph, distinctness is plotted against VelocityVacuum.
From this we can see, that differences in distinctness did not have a
huge affect on the measured Velocities either. The error was likely not
