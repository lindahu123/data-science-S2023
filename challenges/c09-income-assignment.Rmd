...
- Document your observations here.
  - From looking at this graph, it's clear that the 2 person families do have a lower median household income overall than larger families. I feel fairly confident about this because even with the confidence interval, there is very little overlap for many counties. 
  - This same claim is more difficult to make about other categoris, since the intervals are so big and the values are close to each other.
  
- Can you confidently distinguish between household incomes in Suffolk county? Why or why not?
  - I cannot confidently distinguish between household incomese in suffolk counties, because the condidence intervals are big in comparison to the difference in values.
...
- What *overall* trend do you see between `SE` and population? Why might this trend exist?
  - There seems to be a negative correlation between SE and population estimate. We can see this because the highest values for standard error are at the minimum end of population estimate and the reverse is true of larger counties.
  
- What does this *overall* trend tell you about the relative ease of studying small vs large counties?
  - It is relatively easier to confidently make claims while study larger counties. This is because the increased sample size means a smaller SE.
...
The chart above represents how income levels differ for different sized families using box plots. While generally, it looks like as family size goes up the mean income increases slightly the IQRs overlap a lot and there are an enormous amount of outliers.
