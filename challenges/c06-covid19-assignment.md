...
```
    ...
    ## # A tibble: 3,220 × 8
    ##    trash   fips  `Geographic Area Name`   Estima…¹ Annot…² Margi…³ Annot…⁴ ...7 
    ##    <chr>   <chr> <chr>                       <dbl> <chr>   <chr>   <chr>   <lgl>
    ##  1 0500000 01001 Autauga County, Alabama     55200 null    *****   *****   NA   
    ##  2 0500000 01003 Baldwin County, Alabama    208107 null    *****   *****   NA   
    ##  3 0500000 01005 Barbour County, Alabama     25782 null    *****   *****   NA   
    ##  4 0500000 01007 Bibb County, Alabama        22527 null    *****   *****   NA   
    ##  5 0500000 01009 Blount County, Alabama      57645 null    *****   *****   NA   
    ##  6 0500000 01011 Bullock County, Alabama     10352 null    *****   *****   NA   
    ##  7 0500000 01013 Butler County, Alabama      20025 null    *****   *****   NA   
    ##  8 0500000 01015 Calhoun County, Alabama    115098 null    *****   *****   NA   
    ##  9 0500000 01017 Chambers County, Alabama    33826 null    *****   *****   NA   
    ## 10 0500000 01019 Cherokee County, Alabama    25853 null    *****   *****   NA   
    ## # … with 3,210 more rows, and abbreviated variable names ¹​`Estimate!!Total`,
    ## #   ²​`Annotation of Estimate!!Total`, ³​`Margin of Error!!Total`,
    ## #   ⁴​`Annotation of Margin of Error!!Total`
...
```
    ...
    ##    trash   fips  Geogr…¹ Estim…² Annot…³ Margi…⁴ Annot…⁵ ...7  date       county
    ##    <chr>   <chr> <chr>     <dbl> <chr>   <chr>   <chr>   <lgl> <date>     <chr> 
    ##  1 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-24 Autau…
    ##  2 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-25 Autau…
    ##  3 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-26 Autau…
    ##  4 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-27 Autau…
    ##  5 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-28 Autau…
    ##  6 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-29 Autau…
    ##  7 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-30 Autau…
    ##  8 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-03-31 Autau…
    ##  9 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-04-01 Autau…
    ## 10 0500000 01001 Autaug…   55200 null    *****   *****   NA    2020-04-02 Autau…
    ## # … with 2,502,822 more rows, 3 more variables: state <chr>, cases <dbl>,
    ## #   deaths <dbl>, and abbreviated variable names ¹​`Geographic Area Name`,
    ## #   ²​`Estimate!!Total`, ³​`Annotation of Estimate!!Total`,
...
```
    ...
    ## # A tibble: 844 × 5
    ##    date       sd_cases mean_cases sd_deaths mean_deaths
...
df_stats %>% 
  ggplot() +
  geom_point(mapping = aes(x = date, y = mean_cases))
```

![](c06-covid19-assignment_files/figure-gfm/q6-task-1.png)<!-- -->
...
```
    ...
    ## # A tibble: 10 × 2
    ##    county                   max_cases_per100k
    ##    <chr>                                <dbl>
    ##  1 Loving                             192157.
    ##  2 Chattahoochee                       69527.
    ##  3 Nome Census Area                    62922.
    ##  4 Northwest Arctic Borough            62542.
    ##  5 Crowley                             59449.
    ##  6 Bethel Census Area                  57439.
    ##  7 Dewey                               54317.
    ##  8 Dimmit                              54019.
    ##  9 Jim Hogg                            50133.
    ## 10 Kusilvak Census Area                49817.
...
```
    ...
    ##  8 Loving                          980.
    ##  9 Robertson                       980.
    ## 10 Martinsville city               946.

**Observations**: From looking at the max cases, it’s shocking that for
many counties, rates of infections were as high as 50k cases per 100k.
From looking at the data for Loving, Texas, it’s also shocking that
somehow there the reported cases per 100k was actually over 100k. It’s
unclear what happened to the data for this to be the case.

In comparing the counties with max cases to the counties with max
deaths, it’s interesting that there isn’t much overlap. The only county
on both lists is Loving, Texas. I has thought that places with higher
cases would be likely to have more deaths as well. It would be
interesting to look at how rural certain places are, since it’s possible
that other factors (such as proximity to hospitals) might have a larger
factor in death rate than the infection rate.
...
df_normalized %>%
  group_by(date) %>% 
  filter(state %in% c("Oregon", "Florida")) %>%
