...
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg_T is an output variable. It's a very effective predictor because it's part of the average temperature and T_norm is the normalized temperature.
...
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx is in the metadata category. It's meaning is the trial number, which is not a physical variable.
...
-   After running corr.test() on a bunch of the variables, the variables x, I_0, Ll W, T_f, U_0 all seem to display a non-zero correlation.
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   The model from q4 tends to be more accurate, the average distances from data points to the trendline tends to be smaller for that model. From looking at the mode, some of the prediction intervals are close to not overlapping with the trendline.
-   Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
    -   q4 has narrower prediction intervals. both plots are on the same scale and the interval lines for the q4 model are shorter.
