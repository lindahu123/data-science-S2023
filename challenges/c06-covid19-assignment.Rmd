...
**Observations**:
From looking at the max cases, it's shocking that for many counties, rates of infections were as high as 50k cases per 100k. From looking at the data for Loving, Texas, it's also shocking that somehow there the reported cases per 100k was actually over 100k. It's unclear what happened to the data for this to be the case.

In comparing the counties with max cases to the counties with max deaths, it's interesting that there isn't much overlap. The only county on both lists is Loving, Texas. I has thought that places with higher cases would be likely to have more deaths as well. It would be interesting to look at how rural certain places are, since it's possible that other factors (such as proximity to hospitals) might have a larger factor in death rate than the infection rate.
